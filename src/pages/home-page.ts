...
export function renderHomePage(): void {
  ...
  const runSearch = async () => {
    const q = qs<HTMLInputElement>("#global-search")?.value.trim() || "";
    if (!q) return showToast("请输入搜索关键词", "warning");
    const container = qs("#search-results");
    if (container) container.innerHTML = `<div class="card flat">正在搜索药物...</div>`;
    try {
      const result = await searchDrugs({ q });
      if (container) container.innerHTML = `<h3>药物结果：${result.items.length}</h3><div class="list">${result.items.map(renderDrugCard).join("") || "无匹配药物"}</div>`;
    } catch (error) { showToast(error instanceof Error ? error.message : String(error), "danger"); }
  };
  qs("#global-search-btn")?.addEventListener("click", runSearch);
  qs<HTMLInputElement>("#global-search")?.addEventListener("keydown", (e) => { if (e.key === "Enter") void runSearch(); });
}
