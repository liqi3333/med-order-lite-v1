import { importCsvDrugs, importLabelText, importMarkdown, importOcrLabel, importPdfLabel, listPlugins, DrugBatchImportResult, DrugImportResult, ValidationIssue } from "../api/import-api.js";
...
function buildImportBody(saveMode: "preview" | "publish", labelText?: string, sourceType: "manual_entry" | "package_insert" | "ocr" = "manual_entry", extra: Record<string, unknown> = {}): Record<string, unknown> {
...
function showValidationErrors(panel: Element | null, errors: string[]): void {
  if (panel) panel.innerHTML = `<div class="warning-panel"><strong>无法导入</strong><ul>${errors.map((item) => `<li>${escapeHtml(item)}</li>`).join("")}</ul></div>`;
...
async function readFileAsBase64(inputId: string): Promise<{ base64: string; name: string } | null> {
  const file = qs<HTMLInputElement>(inputId)?.files?.[0];
  if (!file) return null;
...
async function runCsvImport(saveMode: "preview" | "publish"): Promise<void> {
  ...
  if (!csvText.trim()) return showToast("请粘贴 CSV/Excel 表格文本或上传 CSV 文件", "warning");
  try {
    if (panel) panel.innerHTML = `<div class="card flat">正在批量解析表格...</div>`;
    ...
    if (panel) panel.innerHTML = renderBatchResult(result);
    showToast(saveMode === "preview" ? "批量预览完成" : `批量导入完成：${result.succeeded}/${result.total}`, result.failed ? "warning" : "success");
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    if (panel) panel.innerHTML = `<div class="warning-panel"><strong>批量导入失败</strong><p>${escapeHtml(message)}</p></div>`;
    showToast(message, "danger");
  }
}
async function runPdfImport(saveMode: "preview" | "publish"): Promise<void> {
  ...
  if (errors.length > 0) return showValidationErrors(panel, errors);
  try {
    if (panel) panel.innerHTML = `<div class="card flat">正在提取 PDF 文本并解析说明书...</div>`;
    ...
    if (panel) panel.innerHTML = renderImportResult(result);
    wireMarkdownCopy();
    showToast(saveMode === "preview" ? "PDF 预览完成" : "PDF 已导入药物库", result.validation.ok ? "success" : "warning");
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    if (panel) panel.innerHTML = `<div class="warning-panel"><strong>PDF 导入失败</strong><p>${escapeHtml(message)}</p><p class="muted">若是扫描型 PDF，请使用图片/OCR 导入，并粘贴 OCR 文本。</p></div>`;
    showToast(message, "danger");
  }
}
async function runOcrImport(saveMode: "preview" | "publish"): Promise<void> {
  const panel = qs("#ocr-import-result");
  const errors = validateCommonBasic();
  const ocrText = valueOf("#ocr-text");
  if (!ocrText.trim()) errors.push("请粘贴 OCR/识别后的说明书文本。");
  const image = await readFileAsBase64("#ocr-file");
  if (errors.length > 0) return showValidationErrors(panel, errors);
  try {
    if (panel) panel.innerHTML = `<div class="card flat">正在解析 OCR 文本...</div>`;
    ...
    const message = error instanceof Error ? error.message : String(error);
    if (panel) panel.innerHTML = `<div class="warning-panel"><strong>OCR 导入失败</strong><p>${escapeHtml(message)}</p></div>`;
    showToast(message, "danger");
  }
}
async function runManualRebuildIndex(): Promise<void> {
  ...
  try {
    ...
    if (button) { button.disabled = false; button.textContent = "手动重建索引"; }
  }
}
...
function wireImportPage(): void {
  ...
  document.querySelectorAll<HTMLButtonElement>("[data-csv-mode]").forEach((button) => button.addEventListener("click", () => void runCsvImport(button.dataset.csvMode as "preview" | "publish")));
  document.querySelectorAll<HTMLButtonElement>("[data-pdf-mode]").forEach((button) => button.addEventListener("click", () => void runPdfImport(button.dataset.pdfMode as "preview" | "publish")));
  document.querySelectorAll<HTMLButtonElement>("[data-ocr-mode]").forEach((button) => button.addEventListener("click", () => void runOcrImport(button.dataset.ocrMode as "preview" | "publish")));
  ...
  qs("#fill-csv-sample")?.addEventListener("click", () => { const csv = qs<HTMLTextAreaElement>("#csv-text"); if (csv) csv.value = SAMPLE_CSV; showToast("已填入 CSV 示例", "info"); });
  qs("#clear-form")?.addEventListener("click", () => { document.querySelectorAll<HTMLInputElement | HTMLTextAreaElement>("input, textarea").forEach((el) => { if (el.type === "checkbox") (el as HTMLInputElement).checked = false; else el.value = ""; }); ["#import-result", "#csv-import-result", "#pdf-import-result", "#ocr-import-result", "#md-import-result"].forEach((id) => { const el = qs(id); if (el) el.innerHTML = ""; }); });
}
