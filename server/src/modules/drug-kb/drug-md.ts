...
const sectionTitleByKey: Record<keyof Omit<DrugLabelSections, "special_populations">, string> = {
  composition: "成分",
  character: "性状",
  indications: "适应症",
  dosage: "用法用量",
...
const specialPopulationTitles: Record<keyof NonNullable<DrugLabelSections["special_populations"]>, string> = {
  pregnancy: "妊娠",
  lactation: "哺乳",
  pediatric: "儿童",
  geriatric: "老年",
...
) as Record<string, keyof NonNullable<DrugLabelSections["special_populations"]>>;

export function parseDrugMarkdown(markdown: string): DrugDocument {
  const match = markdown.match(/^---\s*\n([\s\S]*?)\n---\s*\n?/);
  if (!match) throw new Error("药物 Markdown 缺少 JSON frontmatter。请以 --- 包裹结构化数据。");
...
export function stringifyDrugMarkdown(frontmatter: DrugFrontmatter, label: DrugLabelSections): string {
  const lines: string[] = [];
  lines.push("---");
  ...
  lines.push("");
  lines.push("> 本文件为药物知识库源文件。候选医嘱需由医生结合患者情况最终确认。AI 或系统生成内容不得直接替代临床判断。");
  lines.push("");

  for (const [key, title] of Object.entries(sectionTitleByKey)) {
    const value = label[key as keyof Omit<DrugLabelSections, "special_populations">];
    lines.push(`## ${title}`);
    lines.push("");
  ...
  for (const [key, title] of Object.entries(specialPopulationTitles)) {
    const value = special[key as keyof NonNullable<DrugLabelSections["special_populations"]>];
    lines.push(`### ${title}`);
    lines.push("");
    lines.push(value?.trim() || "待补充");
    lines.push("");
  ...
  if (frontmatter.sources.length === 0) {
    ...
    for (const source of frontmatter.sources) {
      lines.push(`- ${source.source_type}：${source.title || source.source_id}${source.url ? ` (${source.url})` : ""}`);
    }
  }
  lines.push("");
  return `${lines.join("\n").replace(/\n{3,}/g, "\n\n")}\n`;
...
export function parseLabelSections(body: string): DrugLabelSections {
  ...
  let currentSpecialKey: keyof NonNullable<DrugLabelSections["special_populations"]> | null = null;
  let buffer: string[] = [];
  ...
  for (const line of lines) {
    ...
    if (h2) {
      ...
      currentKey = title === "特殊人群" ? "special_populations" : keyBySectionTitle[title] || null;
      continue;
    }
