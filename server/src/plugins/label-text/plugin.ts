...
const headingMap: Array<{ key: LabelKey; patterns: RegExp[] }> = [
  { key: "composition", patterns: [/^成[份分]$/i, /^主要成[份分]$/i, /^活性成[份分]$/i, /^组成$/i] },
  { key: "character", patterns: [/^性状$/i, /^性状特征$/i] },
  { key: "indications", patterns: [/^适应症$/i, /^适应证$/i, /^功能主治$/i, /^主治$/i, /^用途$/i] },
  { key: "dosage", patterns: [/^用法用量$/i, /^用法与用量$/i, /^用量用法$/i, /^给药方法$/i, /^使用方法$/i] },
  { key: "contraindications", patterns: [/^禁忌$/i, /^禁忌症$/i, /^禁用$/i] },
  { key: "precautions", patterns: [/^注意事项$/i, /^警告和注意事项$/i, /^警告$/i, /^注意$/i, /^特别警告$/i] },
  { key: "adverse_reactions", patterns: [/^不良反应$/i, /^副作用$/i] },
  { key: "interactions", patterns: [/^药物相互作用$/i, /^相互作用$/i, /^药物交互作用$/i] },
  { key: "pharmacology_toxicology", patterns: [/^药理毒理$/i, /^药理作用$/i, /^药效学$/i] },
  ...
  { key: "approval_number", patterns: [/^批准文号$/i, /^注册证号$/i, /^进口药品注册证号$/i] },
  { key: "revision_date", patterns: [/^说明书修订日期$/i, /^说明书修订$/i, /^核准日期$/i, /^修订日期$/i] }
];
...
function matchHeading(title: string): LabelKey | null {
  ...
  for (const item of headingMap) {
    if (item.patterns.some((pattern) => pattern.test(normalized))) return item.key;
  }
  return null;
}
...
function matchSpecialHeadings(title: string): SpecialKey[] {
  const normalized = normalizeTitle(title);
  return specialMap.filter((item) => item.patterns.some((pattern) => pattern.test(normalized))).map((item) => item.key);
}

function parseHeadingLine(line: string): { title: string; rest: string } | null {
  ...
  const bracket = trimmed.match(/^[【\[]([^】\]]{1,40})[】\]]\s*[:：]?\s*(.*)$/);
  if (bracket) return { title: bracket[1], rest: bracket[2] || "" };

  const parenthesis = trimmed.match(/^[（(]([^）)]{1,40})[）)]\s*[:：]?\s*(.*)$/);
  if (parenthesis) return { title: parenthesis[1], rest: parenthesis[2] || "" };

  const numbered = trimmed.match(/^(?:第?[一二三四五六七八九十百\d]+[、.．)）]\s*)([^：:]{1,40})(?:[:：]\s*(.*))?$/);
...
function assignLabelValue(label: DrugLabelSections, key: LabelKey, value: string): void {
...
function assignSpecialValue(label: DrugLabelSections, keys: SpecialKey[], value: string): void {
  const cleaned = value.trim();
  ...
  for (const key of keys) {
    ...
    label.special_populations[key] = existing?.trim() ? `${existing.trim()}\n${cleaned}` : cleaned;
  }
}
...
function extractInlineField(text: string, names: string[]): string | undefined {
  for (const name of names) {
    ...
    const bracket = new RegExp(`(?:【${escaped}】|${escaped}[：:])\\s*([^\\n]+)`, "i").exec(text);
...
function extractSections(text: string): DrugLabelSections {
  ...
  function flush(): void {
    ...
    if (currentSpecialKeys.length > 0) assignSpecialValue(label, currentSpecialKeys, value);
    buffer = [];
  }
  ...
  label.approval_number = label.approval_number || extractInlineField(normalizedText, ["批准文号", "注册证号", "进口药品注册证号"]) || "";
  label.revision_date = label.revision_date || extractInlineField(normalizedText, ["说明书修订日期", "核准日期", "修订日期"]) || "";
...
function nonEmptyKeys(label: DrugLabelSections): string[] {
  ...
  for (const [key, value] of Object.entries(label.special_populations || {})) {
    if (typeof value === "string" && value.trim()) result.push(`special_populations.${key}`);
  }
  return result;
}
...
export const labelTextPlugin: DrugImportPlugin<LabelTextImportInput> = {
  id: "label-text",
  name: "说明书文本导入",
  description: "接收粘贴的药品说明书文本，支持【标题】内容、标题：内容、编号标题和 Markdown 标题等格式，生成标准 drug.md 预览或正式药物文件。",
  inputSchema: {
  ...
  async import(input, context) {
    ...
    if (!input.label_text?.trim()) throw new Error("label_text 不能为空，请粘贴药品说明书文本。");

    const id = `drug-${slugify(genericCn)}`;
    ...
    return {
      ...
      notes: [
        ...
        extracted.length > 0 ? `已抽取字段：${extracted.join("、")}` : "未识别到结构化标题，请检查说明书文本是否包含【适应症】【用法用量】等标题。",
